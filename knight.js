...
program.version(require('./package.json').version)
    ...
    .option("-t --type <machine_type>","Specify type of AWS/GCP VM to spin up")
    .option("-i --image <image_name>", "Specify AMI/GCP image to use.")
    .option("-I --identity <identity>", "Specify the path to the identity file to use for SCP and SSH.")
    .option("--tag <tag>", "Tag name to keep track of the instances")
    .option("-k --key <key>", "Public Key used for authorization")
    .option("-s --secret <secret>", "AWS Key Secret used for authorization")
    .option("-r --region <region>", "Cloud region to use")
    .option("-g --gcp", "GCP Mode, currently on by default till AWS integration is complete", true)
