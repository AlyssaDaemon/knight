...
function nodeShell(opts, machines, promise, callbackCalled, callback) {
  ...
  console.log("Ready to siege...\n(type 'help' for commands or 'exit' or use Ctrl-c to quit knight)");
  ...
  rl.on("line", (data) => {
    ...
    if (/^status/.test(input)) {
      console.log(machinesReady + " out of " + machines.size + " machines ready for action!");
      rl.prompt();
      return;
    }
    ...
    if (/^help/.test(input)) {
      console.log("\n\nCommands:");
      console.log("\n'help' shows this screen\n'status' shows how many machines are available");
      console.log("'exit' quits knight (and kills the nodes)");
      rl.prompt();
      return;
    }
