...
function runPDSH(opts, machines, callback) {
  ...
  var sigIntPDSH = () => pdsh.stdin.write("\x03");
  ...
  connectClients(opts,machines, (err) => {
    ...
    if (opts.file) {
      ...
      uploadSiegeFileShell(opts, machines, (err) => {
        //if (err) return callback(err, machines);
        console.warn(`Unable to upload Siege File due to ${err.message || JSON.stringify(err)}`);
...
function nodeShell(opts, machines, promise, callbackCalled, callback) {
  let logFile;
  let lastSIGINT = 0;
  if (opts.outputFile) logFile = fs.createWriteStream(opts.outputFile, {autoClose: false}); 
  let readies = 0;
    ...
    machines.forEach((machine,name) => {
      machine.ssh.exec(input,{ pty: true }, (err, stream) => {
        ...
        stream.on("data", (input) => {
          ...
          if (logFile) logFile.write(name + ": " + input.toString('utf-8'))
        })
        stream.stderr.on("data", (input) => {
          process.stdout.write("E:" + name + ": " + input.toString('utf-8'))
          if (logFile) logFile.write("E:" + name + ": " + input.toString('utf-8'))
        })
        stream.on("end", () => {
          stream.ended = true;
          readies += 1;
