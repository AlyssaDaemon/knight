...
function nodeShell(opts, machines, promise, callbackCalled, callback) {
  ...
  rl.on("line", (data) => {
    ...
    if (/^exit/.test(input)) {
      if (logFile) {
        process.stdout.unpipe(logFile);
        process.stderr.unpipe(logFile);
        logFile.end(); 
        logFile = null;
      }
      ...
      machine.ssh.exec(input,{ pty: true }, (err, stream) => {
        ...
        if (err) {
          ...
          stream.on("data", (input) => {
            ...
            process.stdout.write(name + ": " + data + data[data.length - 1] === "\n" ? "" : "\n");
            //if (logFile) logFile.write(name + ": " + input.toString('utf-8'))
          })
          stream.stderr.on("data", (input) => {
            let data = input.toString('utf-8');
            process.stdout.write("E:" + name + ": " + data + data[data.length - 1] === "\n" ? "" : "\n");
