...
module.exports.generateVMs = function (opts, callback) {
  ...
  async.times(opts.max || opts.min, (idx, cb) => { //idx is unused
      ...
      zone.createVM(name, {
        ...
        http: true,
        scheduling: {
          preemptible: Boolean(opts.preemptible)
        }
      }).then((data) => {
          let vm = data[0],
              operation = data[1],
          ...
          operation.on('complete', metadata => {
            ...
            vm.waitFor("RUNNING", {timeout:600}, (err, metadata) => {
              if (!err) {
                if (opts.verbose > 0) console.log(`${name} created. Waiting ${WAIT_TIME / 1000}s for it to boot.`)
                machine.ip = metadata.networkInterfaces[0].accessConfigs[0].natIP;
              }
              setTimeout(() => cb(err), opts.waitTime || WAIT_TIME);
            });
          });
        }).catch(cb);
...
module.exports.deleteVMs = function deleteVMs(opts, machines, callback) {
